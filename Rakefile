require 'rake'
require 'fileutils'
require File.join(File.dirname(__FILE__), 'bin', 'utils', 'plug')
...
task :install do
  ...
  if RUBY_PLATFORM.include? 'linux'
    if want_to_install? 'compton: composite window manager for Linux'
      FileUtils.ln_sf(
        File.join(ENV['HOME'], '.dotfiles', 'apps', 'compton', 'compton.conf'),
        File.join(ENV['HOME'], '.config', 'compton.conf')
      )
    end

    if want_to_install? 'termite: terminal for linux'
      FileUtils.mkdir_p File.join(ENV['HOME'], '.config', 'termite')
      FileUtils.ln_sf(
        File.join(ENV['HOME'], '.dotfiles', 'apps', 'termite', 'config'),
...
private
  ...
  def install_fonts
    ...
    puts "======================================================"
    run %{ git clone https://github.com/powerline/fonts ~/.dotfiles/fonts}

    install_dir = File.join ENV['HOME'], '.dotfiles', 'fonts'
  ...
  def install_prezto
    ...
    run %{ ln -nfs "$HOME/.dotfiles/cli/zsh/prezto" "${ZDOTDIR:-$HOME}/.zprezto" }
    ...
    run %{ ln -nfs "$HOME/.dotfiles/cli/zsh/prezto-override/zpreztorc" "${ZDOTDIR:-$HOME}/.zpreztorc" }
    run %{ ln -nfs "$HOME/.dotfiles/cli/zsh/prezto-override/zprofile" "${ZDOTDIR:-$HOME}/.zprofile" }
